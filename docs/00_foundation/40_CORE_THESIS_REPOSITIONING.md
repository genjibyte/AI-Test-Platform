...
- 不覆盖、不改 Charter（本文 sharpen + 交叉引用；Charter §1/§2/§6/§11 的"生成中心"措辞，**待你确认后**再回写）。
...
2. 是否允许我把 §5 改述 + §6.1 命名解耦**回写进 Charter**（§1/§2/§6/§11 的措辞）？（涉及修改最高约束文件，需你明确授权。）
3. **沉淀层**（badcase / 失败归因 / 跨作者比较）是否作为下一个**功能**投入方向？（仍先设计后实现。）
